...
handle_binds() {
	...
	bind_select_up=$(tmux_option_or_fallback "@sessionx-bind-select-up" "alt-k")
	bind_select_down=$(tmux_option_or_fallback "@sessionx-bind-select-down" "alt-j")

	bind_claude_preview=$(tmux_option_or_fallback "@sessionx-bind-claude-preview" "alt-c")
	bind_reset_title=$(tmux_option_or_fallback "@sessionx-bind-reset-title" "alt-r")
...
handle_args() {
	...
	WINDOWS_MODE="$bind_window_mode:reload(tmux list-windows -a -F '#{session_name}:#{window_index} #{?@title,#{@title},#{window_name}}' | sed 's|$HOME/||g')+change-preview(${SCRIPTS_DIR%/}/preview.sh -w {1})+change-preview-label( Preview )"
	...
	CLAUDE_PREVIEW="$bind_claude_preview:change-preview(${SCRIPTS_DIR%/}/claude-preview.sh {1})+change-preview-label( Claude )"
	RESET_TITLE="$bind_reset_title:execute-silent(tmux set-option -wu -t {1} @title)+reload(tmux list-windows -a -F '#{session_name}:#{window_index} #{?@title,#{@title},#{window_name}}' | sed 's|$HOME/||g')"
	WINDOWS_PREVIEW="$bind_window_mode:reload(tmux list-windows -a -F '#{session_name}:#{window_index} #{window_name}')+change-preview(${SCRIPTS_DIR%/}/preview.sh -w {1})+change-preview-label( Preview )"
