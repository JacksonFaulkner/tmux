...
# Batch both values in one tmux call
eval "$(tmux display-message -p 'CURRENT="#{session_name}";LAST_SESSION="#{client_last_session}"')"
Z_MODE="off"
SORTED_SESSIONS=""
...
get_sorted_sessions() {
	...
	SORTED_SESSIONS=$(echo -e "$sessions\n$LAST_SESSION" | awk '!seen[$0]++')
	echo "$SORTED_SESSIONS"
}
...
input() {
	...
	if [[ "$default_window_mode" == "on" ]]; then
		tmux list-windows -a -F '#{session_name}:#{window_index} #{?@title,#{@title},#{window_name}}' | sed "s|$HOME/||g"
	else
